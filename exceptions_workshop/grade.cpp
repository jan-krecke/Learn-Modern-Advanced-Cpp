...
class invalid_student_grade : public std::out_of_range {
 ...
 public:
  ...
  // std::exception
  invalid_student_grade(const char* s, int grade)
      : std::out_of_range(s), grade(grade) {}
  invalid_student_grade(const string& s, int grade)
      : std::out_of_range(s), grade(grade) {}

  // The default copy constructor is good enough as we do not have any data
  // members
