...
int main() {
  ...
  vector<int> vec2{1, 2, 2, 2,
                   2, 3, 3};  // The vectors differ at their fourth element
  ...
  if (elems.first != cend(vec1) && elems.second != cend(vec2)) {
    cout << "Found mismatch at index " << distance(cbegin(vec1), elems.first)
         << endl;
    cout << "vec1 has " << *elems.first << ", vec2 has " << *elems.second
         << endl;
  }
}
