...
int main() {
  ...
  if (second != end(str)) cout << "Second element is " << *second << endl;

  auto last = prev(end(str));  // Returns iterator to last element

  if (last != end(str)) cout << "Last element is " << *last << endl;

  // end() - begin() gives the number of elements
  auto mid = begin(str);
  advance(mid, distance(begin(str), end(str)) /
                   2);  // Returns iterator to middle element
