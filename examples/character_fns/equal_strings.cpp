...
bool equal_strings(const string& lhs, const string& rhs) {
  ...
  // We iterate over the two strings, comparing the current character from each
  // string. If either iterator is equal to cend(), we know that we have seen
  // all the characters and the loop terminates. On each iteration, we look for
  // a mismatch. If the character has a different value in the two strings, then
  // we know the strings are different and we can terminate the loop.
...
int main() {
  ...
  cout << a << " and " << b << " are " << (equal_strings(a, b) ? "" : "not ")
       << "equal\n";
  cout << b << " and " << c << " are " << (equal_strings(b, c) ? "" : "not ")
       << "equal\n";
  cout << a << " and " << c << " are " << (equal_strings(a, c) ? "" : "not ")
       << "equal\n";
}
