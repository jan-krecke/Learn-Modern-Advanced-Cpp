...
int main() {
  ...
  for (auto score : scores) print(score);
  cout << endl;

  auto start =
      scores.lower_bound("Graham");  // Find first element with key "Graham"
  auto finish =
      scores.upper_bound("Graham");  // Find first element after "Graham"

  for (auto it = start; it != finish;
       ++it) {  // Loop over the matching elements
    if (it->second == 66) {
