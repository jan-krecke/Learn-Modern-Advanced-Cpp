...
int main() {
  ...
  // seconds sec2 = 5043ms;                               // Error - data would
  // be lost

  seconds s =
      duration_cast<seconds>(5043ms);  // OK - s is truncated to 5 seconds
  cout << "5043ms converted to " << s.count() << " seconds\n";

  seconds s2 =
      duration_cast<seconds>(-5043ms);  // OK - s2 is truncated to -5 seconds
